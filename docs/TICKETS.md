...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — PR #48 (`007884c`) restores urgent-market ordering and bounded read-only `get_news`; PR #49 (`8c8b105`) keeps exact allowlisted PRIMARY evidence reachable under DISCOVERY load. 2,394 tests, independent specification/security PASS, closing 10/10 mutations. Installed into the existing exact-six profile and live-verified: 111 books, first natural evidence-aware cron `ok`, both units active+enabled, zero restarts/swap/pressure/OOM/economic/outbox/raw-WS rows. Honest proposals still depend on separately reviewed market-relevant PRIMARY source coverage | POL-11, POL-12 |
