...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — approved enablement passed, then extended observation found four fail-closed POL-17 restarts (three fixed-universe subset TTL expiries, one resync HALT). Corrective branch publishes only coherent current registry subsets while freezing collector identity and removing omitted/quarantined tokens from ERS and every Hermes read surface using one immutable generation; 2,311 tests pass, review findings are fixed, closing gates/landing/install/restart pending. Services remain paper-only under memory caps; no live-money authority | POL-11, POL-12 |
