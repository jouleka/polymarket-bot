...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — stopped install/config plus native existing-Hermes profile/auth/model, one exact reviewed five-minute cron, and exact-five preflight complete; shared-VPS cgroups cap combined RAM at 1.25 GiB and swap at 256 MiB; no DB/start/enable/activation, both units inactive/disabled; later gates remain | POL-11, POL-12 |
