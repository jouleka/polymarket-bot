...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — PR #37 (`e1e17c1`) corrected the live-attributed ask `1` empty-side sentinel after 2,336 tests/reviews/mutations and passed both prior recurrence windows. PR #39 (`26e2009`) then replaced vulnerable MCP 1.26.0 with 1.28.1 in both existing venvs after 2,338 tests, independent reviews, and 5/5 mutations. Stopped maintenance preserved config/auth, seven databases, raw evidence, exact-five Hermes, and caps; ordered restart exposed 144 live books and the first Hermes turn completed `ok` with zero restarts/swap/economic/outbox/raw/pressure/OOM activity. Both paper/shadow units are active+enabled; no signing or live-money authority | POL-11, POL-12 |
