...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — PR #42 (`e8c51ba`) landed root-only Codex auth persistence with transferred native lock ownership; the exact incident artifact was guardedly removed after stopped installation. PR #43 (`402d44b`) bounded per-shard websocket receive queues. PR #44 (`cb8ff79`) fixed the dominant boot allocation by preventing exact `wallet=None` shadow restart from materializing the 685 MiB EventStore; wallet-present authority is unchanged. Final suite 2,365 with independent specification/security PASS and zero mutation survivors. Original config now runs POL-17 near 90 MiB and Hermes near 257 MiB, zero restart/swap/pressure/OOM; both are active+enabled, exact-five, profile auth/env absent, no raw CLOB websocket persistence, no signer/live-money authority. Natural token-refresh observation remains open | POL-11, POL-12 |
