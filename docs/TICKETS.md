...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — stopped install/config plus native existing-Hermes `polymarket` profile, inherited `openai-codex`, `gpt-5.6-terra` high reasoning, and exact-five preflight complete at service checkout `6ea7e94`; no cron/DB/start/enable/activation, both units inactive/disabled; later gates remain | POL-11, POL-12 |
...
| **POL-18** | deployed isolated propose-only Hermes brain | DONE — exact reviewed build head `5e57449` landed through [PR #9](https://github.com/jouleka/polymarket-bot/pull/9) as merge `09a3a6b`; native-profile correction landed through PR #15; 2,275 tests, independent specification/security PASS, 46/46 isolated mutations; existing-Hermes profile/model/auth preflighted stopped, but no cron/start/enable/activation | facade/MCP deployment; consumed by POL-17 runtime |
