...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — PRs #27/#28 landed live-book and auth-isolation fixes. Retry proved 149 live tokens, non-empty batches, no auth recreation/raw/economic rows/memory pressure, then failed closed on page-wide impact from one unavailable market and lost planned-stop process identity. Checkpoints `a391868`/`7a3b0b9` close both; 2,305 tests and 9/9 mutations pass. Independent closing re-review, landing, stopped install, and retry remain | POL-11, POL-12 |
