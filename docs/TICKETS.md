...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — the fixed-universe registry correction passed its old 900-second window, then observation exposed eight fail-closed order-book resync exits. Reviewed candidate `2c7db2b` closes the stale-generation reconnect race, normal-close/teardown authority gaps, semantic-format bypass, bounded evidence, and single-collector lock lifecycle; 2,330 tests, final specification/security PASS, isolated 10/10 mutations. A stopped 1,800-second 200-token/8-shard probe used ~85 MB and had no natural storm. Both units are inactive/dead but enabled pending landing, stopped install, ordered restart, and observation; no economic/raw/signing/live-money authority | POL-11, POL-12 |
