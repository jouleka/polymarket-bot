...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — stopped install/config/profile plus `gpt-5.6-terra`/`openai-codex` high-reasoning selection and exact-five preflight complete at service checkout `28c3dab`; no model credential/cron/DB/start/enable/activation, both units inactive/disabled; later gates remain | POL-11, POL-12 |
...
| **POL-18** | deployed isolated propose-only Hermes brain | DONE — exact reviewed build head `5e57449` landed through [PR #9](https://github.com/jouleka/polymarket-bot/pull/9) as merge `09a3a6b`; 2,273 tests, independent specification/security PASS, 46/46 isolated mutations; isolated profile created stopped under POL-13, but no model/cron/start/enable/activation | facade/MCP deployment; consumed by POL-17 runtime |
