...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — auth isolation and walletless boot-memory fixes are landed. The extended soak proved natural root-only token persistence and zero process restarts/OOM, then correctly recorded a sticky `l5_ws_down` on one quiet shard while the collector later recovered. Corrective branch `pol-17-ws-silence-resnapshot` proactively reconnects a responsive market-silent shard at 10s, with threshold+ping capped at 20s before unchanged 30s L5, without letting PONG refresh book authority. Strict TDD and zero-survivor mutation review complete; independent re-review/closing suite and landing remain. Services stay paper-only with zero economic rows, zero raw CLOB persistence, and no signer/live-money authority | POL-11, POL-12 |
