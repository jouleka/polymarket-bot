...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — the continued observation exposed Hermes 0.18.2 persisting borrowed Codex OAuth state into forbidden profile-local `auth.json`; both paper/shadow services are now inactive+disabled and the artifact is preserved as non-secret incident evidence. Reviewed candidate `bf9e317` confines native atomic saves to a root-only one-shot writer, transfers/verifies the caller's native lock lease across process death, preserves all non-Codex state, and leaves Hermes without shared-root write access. Canonical 2,363 tests, exact-head security PASS, and zero mutation survivors; publication, stopped installation/cleanup/preflight, and restart remain separate recorded gates. No signing or live-money authority | POL-11, POL-12 |
