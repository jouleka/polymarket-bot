...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — PR #27 landed usable-book readiness and 25-token shards as `9df7c10`; stopped install/config reconciliation passes. Exact-five preflight then caught an unselected Hermes Nous keepalive persisting a profile-local credential. Follow-up `369b477` disables only that thread before the same profile launch; 2,296 tests and 4/4 mutations pass. Independent review, landing, stopped cleanup/preflight, and retry remain | POL-11, POL-12 |
