...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — PR #27 landed usable-book readiness and 25-token shards as `9df7c10`; stopped install/config reconciliation passes. Exact-five preflight then caught an unselected Hermes Nous keepalive persisting a profile-local credential. Follow-up `369b477` disables only that thread before the same profile launch; closing 2,297 tests and 5/5 mutations pass. Independent re-review, landing, stopped cleanup/preflight, and retry remain | POL-11, POL-12 |
