...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — PR #51 merged as `e3f04eb` and is installed in the existing exact-six paper/shadow runtime. Exact White House, UN Middle East, War/Defense, and IAEA PRIMARY feeds use a fixed-memory literal query cache; Google remains DISCOVERY. 2,410 tests, independent specification/security PASS, zero mutation survivors, no EventStore/schema/signer/authority change. Live proof: all four feeds HTTP 200, 186 books, bounded eligible `Iran` results, natural runs 998/999 `ok` with market-specific queries and honest `[SILENT]` on unsupported sports, both units active+enabled within memory ceilings, zero restarts/swap/pressure/OOM/economic/outbox/raw-WS rows. Sports evidence remains a separate integration | POL-11, POL-12 |
