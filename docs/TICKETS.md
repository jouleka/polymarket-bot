...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — PR #30 landed the final helper-interpreter correction as `f07ee06`; approved enablement passed. POL-17/Hermes are active+enabled, 172 live tokens, exactly five tools, successful catch-up and regular read-only cycles, clean planned stop/restart, nonempty midpoint batches, zero raw/economic/outbox/auth-recreation/memory-pressure events, and healthy stores/evidence. Continue the bounded shadow observation window; no live-money authority | POL-11, POL-12 |
