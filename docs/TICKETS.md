...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — PR #46 (`775b057`) fixes the soak's quiet-shard `l5_ws_down`: responsive market-silent shards reconnect at 10s, threshold+ping is capped at 20s before unchanged 30s L5, and PONG cannot refresh book authority. 2,379 tests, independent specification/security PASS, and 7/7 mutations. Installed and reconciled `RUNNING` with 159 live books; both units active+enabled, first Hermes cron `ok`, zero restarts/swap/pressure/OOM, zero economic/outbox/raw-WS rows. Separate follow-up: planned Hermes stop during an in-flight upstream failure returned exit 1 despite successful `ExecStop` and no survivor. No signer/live-money authority | POL-11, POL-12 |
