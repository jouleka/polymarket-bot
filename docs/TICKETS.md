...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — the active exact-five soak exposed a zero-proposal discovery defect: Hermes repeatedly researched arbitrary condition-ID page zero and had no sanitized EventStore evidence/citation read. Reviewed branch head `2f60a99` orders nearest positive deadlines, marks shared live-book availability, and adds bounded read-only `get_news` to the existing profile. 2,390 tests, independent specification/security PASS, and 27/27 mutations; landing/stopped install/exact-six live verification pending. No signer/live-money authority | POL-11, POL-12 |
