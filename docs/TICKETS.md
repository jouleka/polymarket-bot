...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — extended observation found fixed-universe subset TTL restart churn; reviewed correction landed via PR #32 as `1c4d6cb` (2,311 tests, two closing reviews, 10/10 mutations), installed with config/data preserved, and restarted in order. First live refresh passed; POL-17/Hermes are active+enabled under caps with zero new-invocation restarts, economic/outbox/raw/auth/memory-pressure events. Continue bounded paper/shadow observation; no live-money authority | POL-11, POL-12 |
