...
| **POL-18** | deployed isolated propose-only Hermes brain | DONE — the existing native profile/model/auth/cron remains the deployed brain under POL-13. PR #48 changes its reviewed inventory from exact five to exact six by adding only bounded read-only `get_news`; the same profile, selected model/auth, propose-only boundary, and no-signing contract remain. Stopped exact-six installation is complete; activation/live-cycle verification awaits the primary-evidence priority follow-up | facade/MCP deployment; consumed by POL-17 runtime |
