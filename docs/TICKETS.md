...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — stopped install/config plus native existing-Hermes profile/auth/model and exact five-minute cron complete; first-start exposed Gamma filtered pagination/omission behavior, corrected candidate `daf7df6` passes 2,283 tests and 7/7 mutations after closing independent-review findings; both units stopped/disabled, Hermes never started, seven paper DBs preserved; publication/install/retry and later gates remain | POL-11, POL-12 |
