...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — first enablement attempt failed closed because a 200-token shard left every live book stale; zero economic rows/restarts/pressure, clean disable/stop, healthy stores/raw evidence. Corrective head `f18d48a` requires a usable book before readiness and pins 25-token shards; 2,295 tests, two independent reviews, and 8/8 isolated mutations pass with no findings. Landing, stopped install/config reconciliation, and retry remain | POL-11, POL-12 |
