...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — evidence-aligned selection is live through PR #53 (`86fcf95`) and evidence-first candidate priority through PR #54 (`ed0d964`). The production pipeline deterministically rejects categories outside the reviewed PRIMARY coverage set; the exact-six Hermes prompt shortlists at most two live-book candidates and queries evidence before selecting one outcome. 2,415 tests, independent specification/security PASS, 18/18 isolated mutations killed. Natural run 1009 created the first genuine proposal/forecast, then ERS correctly recorded `SKIP/no_edge` because two same-publisher UN citations could not corroborate and fused probability 0.905 was below the 0.91 ask. Run 1010 honestly returned `[SILENT]` without independent evidence. Both units remain active+enabled inside unchanged memory caps with zero restarts/swap; all databases are healthy; zero raw-WS rows, fills, execution outboxes, executions, or shadow trades. Continue observation; never weaken truth/edge gates to manufacture a fill | POL-11, POL-12 |
...
| **POL-18** | deployed isolated propose-only Hermes brain | DONE — the existing native profile/model/auth/cron remains the deployed brain under POL-13. PR #48 adds only bounded read-only `get_news`; PR #49 prioritizes pinned PRIMARY identities without promoting discovery. The same `gpt-5.6-terra` profile, native auth, cron, propose-only boundary, and no-signing contract are active+enabled and live-verified with exact six tools | facade/MCP deployment; consumed by POL-17 runtime |
