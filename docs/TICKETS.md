...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — PR #29 landed row isolation and supervised argv fixes. The next retry proved 110 live tokens and clean planned stop, then failed closed because native argv contaminated `sys.executable` and broke every MCP watchdog launch; zero tools/proposals/economic rows. Strict-TDD `3265cf9` restores the pinned helper interpreter without losing supervised identity; 2,305 tests, 78 focused cases, installed probe, 3/3 mutations, and independent specification/security reviews pass. Landing, stopped install, and retry remain | POL-11, POL-12 |
