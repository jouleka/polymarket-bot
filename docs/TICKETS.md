...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — existing exact-six paper/shadow runtime is active+enabled. Reviewed primary-source candidate `cc1641b` adds exact White House, UN Middle East, War/Defense, and IAEA PRIMARY feeds plus a fixed-memory literal query cache; Google remains DISCOVERY. 2,410 tests, independent specification/security PASS, closing 11/11 survivor mutations with zero survivors, no EventStore/schema/signer/authority change. Candidate is not yet landed or installed; honest proposals remain evidence/edge-gated and sports data is separate | POL-11, POL-12 |
