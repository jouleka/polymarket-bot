...
| **POL-13** | D4a onward — shadow deployment work-stream | IN PROGRESS — stopped install/config plus isolated `polymarket` profile creation complete at service checkout `28c3dab`; no model credential/cron/DB/start/enable/activation, both units inactive/disabled; later gates remain | POL-11, POL-12 |
