...
CODE, not in a prompt:
  ...
  * Read surface: ``get`` / ``audit_log`` (own proposals + the immutable audit)
  plus the 5 Hermes read tools (``get_market`` / ``get_book`` / ``get_news`` /
  ``get_ledger`` / ``get_flags``), each delegating to an INJECTED read-only callable.
...
class ProposeOnlyFacade:
    ...
    # --- the 5 Hermes read tools: delegate to injected read-only callables ---
    ...
    def get_news(self, *args, **kwargs):
        return self.__news_reader(*args, **kwargs)
