...
class ShadowLedger:
    ...
    def apply_shadow_execution(self, execution):
        """Project one typed POL-16 paper execution into the shadow ledger."""
        ...
        try:
            changed = self.record_trade(
                ...
                fill_price=execution.price_exec,
        ...
        if not changed:
            row = self._query(
                f"SELECT {_COLUMNS} FROM shadow_trades WHERE trade_id=?",
                (execution.execution_id,),
            )[0]
            actual = (
                row.token_id, row.condition_id, row.category, row.side, row.shares,
                row.fill_price, row.fill_mid, row.reward_accrued, row.event_id,
            ...
            if actual != expected:
                raise SettlementConflict("shadow row contradicts shadow execution")
        return changed
    ...
    def _apply_shadow_execution_after_terminal(self, execution):
        ...
        try:
            ...
            if receipt is None:
                raise SettlementConflict("shadow terminal receipt disappeared during replay")
            ...
            if (subject.event_id != execution.event_id
                ...
                raise SettlementConflict("shadow terminal subject contradicts shadow execution")
            ...
            if actual != expected:
                raise SettlementConflict("shadow row contradicts terminal shadow execution")
        except Exception:
            self._conn.rollback()
            raise
        else:
    ...
    def apply_terminal(self, terminal):
        ...
        try:
            ...
            stored_rows = self._conn.execute(
                ...
                "resolution_numerator, resolution_denominator, terminal_id FROM shadow_trades "
                "WHERE condition_id=?",
                (terminal.subject.condition_id,),
            ).fetchall()
            rows = []
            for (trade_id, token_id, category, event_id, slot, sibling_json, status,
                ...
                if all(value is None for value in identity):
                    if row_terminal_id is not None:
                        raise SettlementConflict("shadow terminal state lacks identity")
                    continue
                ...
                if row_terminal_id is None:
                    if any(value is not None for value in (
                            ...
                            stored_denominator)):
                        raise SettlementConflict("shadow pending row has settled state")
                    rows.append((trade_id, slot))
                    continue
                if receipt is None:
                    raise SettlementConflict("shadow settled row has no terminal receipt")
                ...
                if (row_terminal_id != terminal_id or status != expected_status
                    ...
                    raise SettlementConflict("shadow settled projection contradicts terminal")
            if receipt is not None:
                if rows:
                    raise SettlementConflict("shadow receipt coexists with pending rows")
                self._conn.commit()
                return 0
            self._conn.execute(
